var GameMachineSale = artifacts.require("GameMachineSale");
var GameMachineToken = artifacts.require("GameMachineToken");

contract('GameMachineSale', function(accounts) {


    beforeEach(async function() {
        this.founder = accounts[0];

        this.startDate = Math.round((Date.now()-24*60*60)/1000);
        ...
        this.testingTransactionAmount = 3;

        this.rate = 1000;
        this.testedBonus = 0.15;

        this.token =  await GameMachineToken.new();
        this.sale = await GameMachineSale.new(this.token.address, this.multisig, this.restricted, this.startDate, this.period);
    ...
    it("Should send token to purchaser", async function() {
        ...
        await this.sale.sendTransaction({value: this.testingTransactionAmount * 10 ** 18, from: accounts[3]});

        const balance = await this.token.balanceOf(accounts[3]);

        assert.equal(balance.valueOf(), this.testingTransactionAmount * ( this.rate + this.rate * this.testedBonus) );
    });

    it("Should be change multisig balance in ether", async function() {
        const balance = await web3.eth.getBalance(this.multisig);

        assert.equal(balance.valueOf(), this.multisigStartBalance.add(this.testingTransactionAmount * 10 ** 18).valueOf());
    });
